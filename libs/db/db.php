...
class Db {
    ...
    public static function connect($dsn, $username=false, $password=false, $driver_options=null) {
        if (!self::$PDOInstance) {
            try {
                self::$PDOInstance = new PDO($dsn, $username, $password, $driver_options);
            }catch(PDOException $ex) {
                die ("PDO CONNECTION ERROR: " . $ex->getMesssage() . "<br/>");
            }
        }
        return new Db();
    }
    ...
    public function insert($sql, $obj) {
        echo $sql . "<br/>";
        $statement = self::$PDOInstance->prepare($sql);
        print "<pre>";
        var_dump($obj);
        print "</pre>";
        foreach (ActiveRecord::$describe as $k => $v) {
            echo $v["Field"] . "=" . $obj->$v["Field"];
            $statement->bindParam($v["Field"], $obj->$v["Field"]);
        }

        $result =  $statement->execute();
        if (!$result) {
            echo '<pre>ERROR: ' . ($statement->errorInfo()) . '</pre>';
        }
        return $result;
    }
}
